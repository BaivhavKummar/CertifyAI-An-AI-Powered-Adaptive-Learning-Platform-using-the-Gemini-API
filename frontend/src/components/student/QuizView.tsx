    ...
    <div className="max-w-3xl mx-auto p-6 bg-gray-800 rounded-xl shadow-2xl">
      <div className="mb-4">
        <div className="flex justify-between items-center mb-2">
            <h2 className="text-xl font-bold text-purple-300">{topic.name} Quiz</h2>
        ...
        {currentQuestion.questionType === QuestionType.MCQ && currentQuestion.options?.map((option, index) => {
          ...
          return (
            <button
              ...
              disabled={isAnswered}
              className={`w-full text-left p-4 rounded-lg text-lg transition-all duration-200 ${buttonClass} ${isSelected && !isAnswered ? 'ring-2 ring-purple-400' : ''}`}
            >
              {option}
            </button>
          )
